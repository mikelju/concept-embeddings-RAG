...
```
docs/plans/
├── 0_master_plan.md          # Visión global de todas las fases
├── phase_1/
│   ├── 1.spec.md              # Especificación funcional (QUÉ + POR QUÉ)
│   ├── 1.0_phase_name.md     # Plan de implementación (CÓMO)
...
│   └── ...
└── fixes/
    ├── fix-1_name.md        # Bug fix o correctivo puntual (correlativo global)
    └── fix-2_name.md
```
...
| `X.0_name.md` | Plan principal de la fase X. Se crea al iniciar la fase, DESPUÉS de la spec. |
| `X.tasks.md` | Tareas atómicas. OPCIONAL — solo si la fase tiene >10 pasos. |
| `X.Y_name.md` | Desviación, problema o ajuste fuera del plan principal. Y es correlativo (1, 2, 3...). |
| `fixes/fix-N_name.md` | Bug fix o correctivo puntual entre fases o dentro de una fase. N es correlativo global. |
...
### Plan de Fase (`X.0_name.md`) — fuente de verdad de la fase
...
### Documento de Desviación (`X.Y_name.md`)
- Se crea cuando:
...
### Fix o correctivo puntual (`fixes/fix-N_name.md`)
...
1. **Leer el plan maestro** (`docs/plans/0_master_plan.md`) para entender el contexto.
...
3. **Verificar si existe la spec funcional** (`docs/plans/phase_X/X.spec.md`).
...
4. **Verificar si existe el plan de fase** (`docs/plans/phase_X/X.0_name.md`).
